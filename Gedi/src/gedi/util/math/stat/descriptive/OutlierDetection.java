...
public class OutlierDetection {
	...
	public OutlierDetection(double[] data) {
		this.data = data;
	}

	public boolean[] outliers() {
		boolean[] re = new boolean[data.length];
		outliersPeirce(re);
		return re;
	}
	
	public void outliersInterquartialRange(boolean[] re) {
		outliersInterquartialRange(re, 1.5);
	}
	public void outliersInterquartialRange(boolean[] re, double cutoff) {
		double[] musd = new double[2];
		boolean changed = true;
		while (changed) {
			int outliers = 0;
			changed = false;
			computeMeanSd(data, re, musd);
			int doubt;

			do {
				doubt = outliers+1;
				double R = getPeirceR(data.length, doubt);
				double max = R*musd[1];
				for (int i=0; i<data.length;i++)
					if (!re[i] && Math.abs(data[i]-musd[0])>max) {
						re[i] = true;
						outliers++;
						changed=true;
					}
			} while (doubt<outliers+1);
		}
	}

	public void outliersPeirce(boolean[] re) {
		double[] musd = new double[2];
		boolean changed = true;
		while (changed) {
			int outliers = 0;
	...
	private int computeMeanSd(double[] data, boolean[] outliers, double[] re) {
		re[0]=0;
		int n = 0;
		for (int i=0; i<data.length; i++)
			if (!outliers[i]){
		...
		if (n>1){
			...
			re[1] = Math.sqrt((accum-(accum2*accum2/n))/(n-1));
		}
		return n;
	}

	private static double getPeirceR(int obs, int doubt) {
		return doubt-1<peirceTable[obs].length?peirceTable[obs][doubt-1]:-1;
	}


	private static double[][] peirceTable = {
		{},
		{},
		{},
		{1.196},
		...
		{1.878,1.57,1.38,1.237,1.114},
		{1.925,1.619,1.43,1.289,1.172,1.059},
		{1.969,1.663,1.475,1.336,1.221,1.118,1.009},
		{2.007,1.704,1.516,1.379,1.266,1.167,1.07},
