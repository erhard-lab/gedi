...
public final class DivSufSort implements ISuffixArrayBuilder
{
    ...
    {
        if (input == null) throw new RuntimeException("input must not be null");
        if (length < 2) throw new RuntimeException( "input length must be >= 2");
        MinMax mm = Tools.minmax(input, start, length);
        if (mm.min < 0) throw new RuntimeException("input must not be negative");
        if (mm.max >= ALPHABET_SIZE) throw new RuntimeException("max alphabet size is " + ALPHABET_SIZE);
