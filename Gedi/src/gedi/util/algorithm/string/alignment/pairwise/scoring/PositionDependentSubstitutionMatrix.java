...
/**
 * Dependent on first sequence!
 * @author erhard
 *
 */
public class PositionDependentSubstitutionMatrix extends AbstractScoring<CharSequence, int[]> implements LongScoring<CharSequence> {
	...
	public void build(char[] chars, float[][][] matrix, int precision) {
		...
		values = new long[matrix.length][indexToChar.length][indexToChar.length];
		for (int p=0; p<matrix.length; p++)
			for (int i=0; i<indexToChar.length; i++)
				for (int j=0; j<indexToChar.length; j++)
					values[p][i][j] = (long)Math.round(matrix[p][i][j]*mult);
	}
	...
	public long getLong(int i, int j) {
		return values[i][s1[i]][s2[j]];
	}
	
	public float getFloat(int i, int j) {
		return matrix[i][s1[i]][s2[j]];
	}
