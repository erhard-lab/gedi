...
public class DoubleDiskSuccinctRmaxq {
    ...
    public static void create(PageFileWriter writer, double[] a) throws IOException {
    	...
    	double[] rp = new double[s+1];   // rp: rightmost path in Cart. tree
    ...
    public DoubleDiskSuccinctRmaxq(String file) throws IOException {
		this(new PageFileView(new PageFile(file)));
	}
    public DoubleDiskSuccinctRmaxq(PageFileView file) throws IOException {
  		this.file = file;
    	...
    	nsb = (n-1)/sprimeprime+1; // number of superblocks
  		...
  		this.Mp = this.ap+n*Double.BYTES;
    ...
    private final double geta(int i) throws IOException {
    	return file.getDouble(ap+Double.BYTES*i);
    }
