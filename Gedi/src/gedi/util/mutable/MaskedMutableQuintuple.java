...
public class MaskedMutableQuintuple<T1, T2, T3, T4, T5> extends MutableQuintuple<T1, T2, T3, T4, T5> {
	
	private static final int FULL_MASK = (1<<5)-1;
	
	private int mask;
	
	public MaskedMutableQuintuple() {
		this(null,null,null,null,null,FULL_MASK);
	}
	public MaskedMutableQuintuple(T1 i1, T2 i2, T3 i3, T4 i4, T5 i5) {
		this(i1,i2,i3,i4,i5,FULL_MASK);
	}
	public MaskedMutableQuintuple(T1 i1, T2 i2, T3 i3, T4 i4, T5 i5, int mask) {
	...
	public boolean equals(Object obj) {
		...
		else if (obj instanceof MaskedMutableQuintuple) {
			MaskedMutableQuintuple p = (MaskedMutableQuintuple) obj;
	...
	@Override
	public int compareTo(MutableQuintuple<T1,T2,T3,T4,T5> o) {
		int re = 0;
	...
	public MaskedMutableQuintuple<T1,T2,T3,T4,T5> clone() {
		return new MaskedMutableQuintuple<T1,T2,T3,T4,T5>(Item1,Item2,Item3,Item4,Item5,mask);
	}
}
