...
public class TranscriptRegionStorageDisplayTemplateGenerator implements AutoTemplateGenerator<PreloadInfo<?, GenomicRegionStoragePreload>,TranscriptRegionStorageDisplayTemplateGenerator> {
	...
	public TranscriptRegionStorageDisplayTemplateGenerator(String path) {
		this.path = path;
	}

	@Override
	public void accept(TemplateEngine t, TranscriptRegionStorageDisplayTemplateGenerator[] g) {
		Runnable res = t.save("paths","id");
		t.set("paths", EI.wrap(g).map(te->te.path).toArray(String.class));
		t.set("id", path);
		t.template("transcriptgenomicregion");
		res.run();
	}
	...
	public double applyAsDouble(PreloadInfo<?, GenomicRegionStoragePreload> pre) {
		...
		if (value.getType()==Transcript.class) return 10;
		if (Transcript.class.isAssignableFrom(value.getType())) return 1;
		return -1;
	}
