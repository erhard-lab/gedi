...
import jline.console.helper.DefaultHelpHandler;
import jline.console.helper.HelpHandler;
...
public class ConsoleReader
{
    ...
    public String readLine(String prompt, final Character mask, String buffer) throws IOException {
        ...
        /*
         * Count how often tab has been pressed: First press, complete but do not show potential multi completions;
         * do this at the second (or third, or fourth ...)
         */
        int tabCount = 0;
    ...
    private final List<Helper> helpers = new LinkedList<Helper>();
    private HelpHandler helpHandler = new DefaultHelpHandler();
    ...
    /**
     * Add the specified {@link jline.console.helper.Helper} to the list of handlers for tab-completion.
     *
     * @param helper the {@link jline.console.helper.Helper} to add
     * @return true if it was successfully added
     */
    public boolean addHelper(final Helper helper) {
        return helpers.add(helper);
    }

    /**
     * Remove the specified {@link jline.console.helper.Helper} from the list of handlers for tab-completion.
     *
     * @param helper     The {@link Helper} to remove
     * @return              True if it was successfully removed
     */
    public boolean removeHelper(final Helper helper) {
        return helpers.remove(helper);
    }

    /**
     * Returns an unmodifiable list of all the helpers.
     */
    public Collection<Helper> getHelpers() {
        return Collections.unmodifiableList(helpers);
    }
    
    public void setHelpHandler(final HelpHandler handler) {
        this.helpHandler = checkNotNull(handler);
    }
     ...
     * @return true if successful
     */
    protected boolean complete(int tabCount) throws IOException {
        // debug ("tab for (" + buf + ")");
        ...
        if (candidates.size()>0) {
        	return getCompletionHandler().complete(this, candidates, position, tabCount);	
        }
        
        String help = null;
        for (Helper helper : helpers) {
        	help = helper.help(bufstr,cursor);
        	if (help!=null) break;
        }

        return help!=null && getHelpHandler().help(this, help, tabCount);
